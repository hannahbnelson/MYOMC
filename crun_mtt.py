...
if __name__ == "__main__":
    ...
    parser.add_argument("--keepGEN", action='store_true', help="Keep GEN")
    ...
    if args.outEOS:
        ...
        eos_prefix = "root://cmsxrootd.crc.nd.edu/"
...
""")
        command = f"source $_CONDOR_SCRATCH_DIR/mtt_run.sh {args.name} $_CONDOR_SCRATCH_DIR/{os.path.basename(fragment_abspath)} {args.nevents_job} $(($1+{args.seed_offset})) {args.max_nthreads} "
        if args.pileup_file:
    ...
    files_to_transfer = [fragment_abspath, 
                            f"{MYOMCPATH}/campaigns/{args.campaign}/mtt_run.sh", 
                        ]
    if args.pileup_file:
